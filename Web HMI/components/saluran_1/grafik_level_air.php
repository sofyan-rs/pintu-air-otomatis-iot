...
$level_air = $mysqli->query("SELECT level_air FROM saluran_1 WHERE id >= '$first_id' AND id <= '$last_id' ORDER BY id ASC");
?>

<canvas id="chartLevelAir1"></canvas>
<script>
    var canvas = document.getElementById("chartLevelAir1").getContext('2d');
    var data = {
        labels: [
            <?php 
    ...
    var chartLevelAir1 = new Chart(canvas, {
        type: 'line',
        data: data,
        options: option
    });
