...
$kelembaban_tanah = $mysqli->query("SELECT kelembaban_tanah FROM saluran_2 WHERE id >= '$first_id' AND id <= '$last_id' ORDER BY id ASC");
?>

<canvas id="chartKelembabanTanah2"></canvas>
<script>
    var canvas = document.getElementById("chartKelembabanTanah2").getContext('2d');
    var data = {
        labels: [
            <?php 
    ...
    var chartKelembabanTanah2 = new Chart(canvas, {
        type: 'line',
        data: data,
        options: option
    });
